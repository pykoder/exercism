...
namespace estate_executor {
    int assemble_account_number(int secret_modifier){
        auto bank_number = khan::bank_number_part(secret_modifier) + garcia::bank_number_part(secret_modifier) + zhang::bank_number_part(secret_modifier);
        return bank_number;
    }
    int assemble_code(){
        return (khan::red::code_fragment() + zhang::red::code_fragment() + garcia::red::code_fragment()) * 
            (khan::blue::code_fragment() + zhang::blue::code_fragment() + garcia::blue::code_fragment());
    }
}