 ...
 * @param {number[]} stack
 *
 * @returns {boolean} true if all cards are even
 */
export function isEachCardEven(stack) {
  return stack.every((card) => (card&1) == 0);
}
...
export function doesStackIncludeOddCard(stack) {
  return stack.some((card) => (card&1) == 1);
}
...
export function getFirstOddCard(stack) {
  return stack.find((card) => (card&1) == 1);
}
...
export function getFirstEvenCardPosition(stack) {
  return stack.findIndex((card) => (card&1) == 0);
}
